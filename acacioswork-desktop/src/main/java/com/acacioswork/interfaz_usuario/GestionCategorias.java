...
public class GestionCategorias extends JPanel {
    ...
    private void cargar() {
        ...
        try {
            Categoria[] arr = ApiClient.get("/categorias", Categoria[].class);
            for (Categoria c : arr) {
    ...
    private void agregar() {
        ...
        Object[] message = { "Nombre de la Categoría:", txtNombre };
        int option = JOptionPane.showConfirmDialog(this, message, "Nueva Categoría", JOptionPane.OK_CANCEL_OPTION);
        if (option == JOptionPane.OK_OPTION) {
            try {
                Categoria c = new Categoria();
                c.setNombre(txtNombre.getText());
                ApiClient.post("/categorias", c, Categoria.class);
                cargar();
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Error: " + e.getMessage());
            }
    ...
    private void eliminar() {
        ...
        if (JOptionPane.showConfirmDialog(this, "¿Eliminar categoría?") == JOptionPane.YES_OPTION) {
            try {
