...
describe("buildTools", () => {
  it("registers the 6 free-tier tools (incluye captureLead + pauseSuspectedBot)", () => {
  ...
  it("pro tier has the 6 base tools plus the 2 Pro tools", () => {
  ...
  it("los giros de cita también capturan leads en free (captureLead + agendarCita)", () => {
    ...
    expect(tools.captureLead).toBeDefined();
    expect(tools.agendarCita).toBeDefined();
    expect(tools.scheduleAppointment).toBeUndefined(); // sigue siendo Pro
  });
