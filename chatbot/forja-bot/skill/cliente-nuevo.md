...
4. **Sé honesto con el logo.** El panel muestra el **nombre** del cliente y el bot habla con su
   marca — eso sí existe. **No prometas un cambio de logotipo dentro del panel**: hoy no es una
   opción del tablero, y meterlo sería tocar código frágil (mejor no, o coméntalo aparte). No
   ofrezcas funciones que el bot no tiene.
...
- **No prometas funciones que no existen** (ej. subir un logotipo en el panel). Adáptate a lo
  que el bot sí hace y dilo honesto.
...
`/configurar-mi-chatbot` (PASO 3), aplica marca + `/voz-de-marca` (PASO 4), pide el "sí" para
publicar y prueba 1 canal (PASO 5), y cierra con checklist + `/cotizar`/`/cobrar` (PASO 6).
