...
PIDE CONFIRMACIÓN antes de: reescribir `member/system-prompt.local.ts`, borrar contenido de
la KB, tocar CUALQUIER cosa en `src/`, instalar dependencias, o un cambio que afecte a más
de un objetivo a la vez.
Mapa fallo → arreglo: inventa → endurece la regla "no inventes" del system prompt o mete el
...
pide permiso para system prompt/src/instalar), y entrega la boleta. No hagas deploy ni git push.
