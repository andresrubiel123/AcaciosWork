...
## Qué se conserva
`forjabot update` sobrescribe SOLO el código del template (`src/`, skills, config base) y **conserva intacto** lo tuyo: la carpeta `member/` (datos del negocio + KB), los secrets de Cloudflare y los datos en D1 (conversaciones, leads). No se pierde nada.
