...
export const CURATED_MODELS: { id: string; label: string; provider: LlmProvider }[] = [
  ...
  { id: "gemini-2.5-flash", label: "Gemini 2.5 Flash · equilibrado y económico", provider: "google" },
...
export function createModel(env: Env, tier: Tier, ov?: LlmOverrides): ResolvedModel {
  ...
  if (provider === "openai") {
    const openai = createOpenAI({ apiKey, baseURL: env.OPENAI_BASE_URL || undefined });
