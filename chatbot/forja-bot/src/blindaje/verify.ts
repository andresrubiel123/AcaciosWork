...
const SAFE_REPLY_BY_LANG: Record<string, string> = {
  es: "Esa me la confirma el equipo — dame un momento y te digo bien, para no darte un dato equivocado.",
  en: "Let me double-check that with the team so I don't give you the wrong info — I'll get back to you in a moment.",
  pt: "Deixa eu confirmar isso com a equipe para não te passar um dado errado — já te retorno.",
};

export function safeConfirmReply(lang: string | undefined): string {
  return SAFE_REPLY_BY_LANG[(lang ?? "es").toLowerCase()] ?? SAFE_REPLY_BY_LANG.es;
}
