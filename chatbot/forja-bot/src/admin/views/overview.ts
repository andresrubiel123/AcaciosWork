  ...
  const gettingStarted = isNewBot
    ...
    <section class="card p-[18px]" style="animation-delay:0s;border:1px solid var(--accent);background:linear-gradient(160deg,var(--accent-soft),var(--panel))">
      ...
      <p class="text-[12.5px] text-muted mb-4" style="line-height:1.5;max-width:640px">
        Tu bot está instalado pero todavía no ha atendido a nadie. Con estos 3 pasos lo dejas listo para recibir clientes:
      </p>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-[10px]">
        ${gettingStartedStep(1, "plug-zap", "/admin/conexiones", "Conecta un canal", "WhatsApp, Instagram o el que uses — ahí es donde tu bot va a hablar con tus clientes.")}
        ${gettingStartedStep(2, "book-open", "/admin/kb", "Llena su conocimiento", "Sube o escribe la info que necesita saber para contestar bien (precios, horarios, servicios…).")}
        ${gettingStartedStep(3, "sliders-horizontal", "/admin/config", "Revisa la configuración", "Ajusta su tono, el modelo de IA y qué tools tiene activas antes de dejarlo correr solo.")}
      </div>
    </section>`
    : "";
