...
export async function renderKbList(
  ...
  const emptyGuide = docs.length
    ...
    <div class="bg-panel border border-line" style="padding:20px 22px;margin-bottom:16px">
      <h3 class="font-display font-semibold text-[13.5px] text-cream" style="margin-bottom:6px">¿Qué es esto y para qué sirve?</h3>
      <p class="text-muted text-[12.5px]" style="line-height:1.6;margin-bottom:8px">
        La base de conocimiento es <b class="text-cream">la memoria de tu negocio</b>: horarios, ubicación,
      ...
      <p class="text-dim text-[11.5px]" style="line-height:1.6;margin-bottom:16px">
        Puedes crear cada documento a mano con <b class="text-cream">＋ Nuevo documento</b>, o dejar que
        Claude Code te entreviste y lo llene por ti:
      </p>
      ${renderConfigPrompt(
        "Ayúdame a llenar la base de conocimiento de mi bot de Forja: pregúntame por mi negocio por secciones (horarios, ubicación, servicios/productos, precios, políticas, preguntas frecuentes) y ve agregando cada cosa a la KB para que el bot responda con esa info. Confírmame cuando ya tenga lo esencial.",
        "Copia este prompt y pégaselo a Claude Code. Te preguntará por tu negocio sección por sección y él va llenando la KB.",
      )}
    </div>`;
