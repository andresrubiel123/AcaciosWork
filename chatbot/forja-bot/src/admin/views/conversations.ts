...
export async function renderInboxList(env: Env, p: InboxParams): Promise<string> {
  ...
  const items = rows
    ...
    (p.filter
      ? `<div style="padding:32px 16px;text-align:center;font-size:12.5px;color:var(--dim)">Sin conversaciones con este filtro.</div>`
      : `<div style="padding:44px 16px;text-align:center">
           ...
           <div style="font-size:12.5px;color:var(--muted);font-weight:600">Aún no hay conversaciones</div>
           <div style="font-size:11.5px;color:var(--dim);margin-top:5px;line-height:1.5">Aquí verás cada chat en cuanto tu bot empiece a atender clientes.</div>
         </div>`);
  ...
  if (p.selectedId) {
    ...
    rightPane = `
      <div class="flex-1 flex flex-col items-center justify-center text-center gap-2" style="background:var(--bg);padding:0 28px">
        ...
        <div style="font-size:12.5px;color:var(--muted);font-weight:600">Aquí vivirá tu bandeja</div>
        <div style="font-size:11.5px;color:var(--dim);line-height:1.5;max-width:300px">Cuando tu bot conteste su primer mensaje, la conversación aparece en la lista de la izquierda y puedes verla y responder desde aquí.</div>
      </div>`;
  } else {
    rightPane = `
