...
export async function renderLeads(env: Env): Promise<string> {
  ...
  const empty = `<div style="padding:48px 18px;text-align:center">
    ...
    <div class="text-[13px]" style="color:var(--muted);font-weight:600">Aún no hay ${esc(niche.recordPlural.toLowerCase())}</div>
    <div class="text-dim text-[11.5px]" style="margin:5px auto 0;line-height:1.5;max-width:380px">Aquí aparecerá cada ${esc(niche.recordSingular.toLowerCase())} que tu bot detecte en una conversación — se llena solo con el uso.</div>
  </div>`;
  const header = cols
