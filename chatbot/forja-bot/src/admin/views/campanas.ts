...
export async function renderCampanas(
  ...
  const templateSection =
    templates.length > 0
      ? `<select name="template_sid" style="width:100%;background:var(--panel);border:1px solid var(--line);color:inherit;padding:9px 10px;font-size:12px">
        ...
        <input name="template_vars" placeholder='Variables JSON opcional, ej {"1":"Santi"}' class="font-mono"
  ...
  const guide = firstRun
    ...
    <div style="border:1px solid var(--line);background:var(--panel);padding:16px 18px;margin-bottom:18px;max-width:860px">
      <div class="text-cream" style="font-size:14px;font-weight:700;margin-bottom:6px">📣 Difusiones y seguimientos por WhatsApp</div>
      <p class="text-dim" style="font-size:12.5px;line-height:1.6;margin:0 0 10px">
        Desde aquí le mandas un mensaje a un <b>segmento</b> de tu base (leads calientes, clientes,
      ...
      <p class="text-dim" style="font-size:12px;line-height:1.6;margin:0 0 14px">
        Para arrancar necesitas <b>WhatsApp conectado</b> y al menos una <b>plantilla aprobada</b>
        (revísalo en la tab <b>Plantillas</b>). Si aún no las tienes, deja que Claude Code te guíe:
      </p>
      ${renderConfigPrompt(
        "Ayúdame a lanzar campañas de WhatsApp con mi bot de Forja: verifica que WhatsApp esté conectado y que tenga una plantilla HSM aprobada para difusión fuera de la ventana de 24h (revisa la tab Plantillas), guíame a segmentar mi base de clientes y a crear y enviar la campaña.",
        "Pégaselo a tu agente (Claude Code) y él deja lista la conexión, la plantilla y tu primera campaña.",
      )}
    </div>`
    : "";
