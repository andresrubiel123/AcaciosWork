...
function areaChart(points: { label: string; value: number }[], width = 640, height = 150): string {
  if (points.length === 0) {
    ...
    <div class="py-8 text-center">
      <p class="text-[12.5px] text-dim">Aún no hay actividad para graficar.</p>
      <p class="text-[11px] text-dim mt-1">Esta gráfica se llena sola en cuanto tu bot empiece a recibir mensajes.</p>
    </div>`;
  }
  const pad = 12;
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-[14px]">
        <div class="card bg-panel border border-line p-[18px]">
          ...
          <table class="w-full text-[12.5px]"><tbody>
            ${channels.map((c) => `<tr style="border-top:1px solid var(--line)"><td class="py-2.5 text-cream">${esc(channelLabel(c.channel))}</td><td class="text-right text-muted text-[11px]">${c.n}</td></tr>`).join("") || `<tr><td class="py-3 text-dim text-[12.5px]">Aún sin datos — se llena con las conversaciones de tus clientes.</td></tr>`}
          </tbody></table>
        </div>
        <div class="card bg-panel border border-line p-[18px]">
          <div class="font-display font-semibold text-[14px] mb-2.5">Tools más usadas</div>
          <table class="w-full text-[12px]"><tbody>
            ${tools.filter((t) => t.tool).map((t) => `<tr style="border-top:1px solid var(--line)"><td class="py-2.5 text-accent2">${esc(t.tool)}</td><td class="text-right text-muted text-[11px]">${t.n}</td></tr>`).join("") || `<tr><td class="py-3 text-dim text-[12.5px]">Aún sin tool calls registradas — aquí verás qué acciones usa tu bot más seguido.</td></tr>`}
          </tbody></table>
        </div>
      </div>
    </div>`;
