    ...
    .map((ch) => {
      ...
      const missing = ch.ok
        ...
        : `<div style="border-left:2px solid var(--bad);padding:2px 0 2px 10px;display:flex;flex-direction:column;gap:4px">
             <div class="text-[11.5px]" style="color:var(--bad);font-weight:600">Falta configurar: <span class="font-mono" style="font-weight:500">${ch.missing
               .map(esc)
               .join(", ")}</span></div>
          ...
          ? `<div style="display:flex;flex-direction:column;gap:6px">
               ...
               <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                 ...
                 <button type="button" class="ghostbtn font-mono" style="font-size:10.5px;border:1px solid var(--line);color:var(--cream);padding:6px 10px;cursor:pointer;background:none;display:inline-flex;align-items:center;gap:5px;white-space:nowrap;flex:none"
                         onclick="navigator.clipboard.writeText('${esc(base + ch.webhookPath)}');this.innerHTML='✓ copiado'">
                   <i data-lucide="copy" width="12" height="12"></i>copiar
                 </button>
               </div>
             </div>`
          : "";
  ...
  const emptyGuide =
    connected === 0
      ...
      <div style="border:1px solid var(--line);background:var(--panel);padding:16px 18px;display:flex;flex-direction:column;gap:8px">
        <div class="text-cream" style="font-size:13px;font-weight:600">Aún no conectas ningún canal 👇</div>
        <p class="text-muted text-[12.5px]" style="margin:0;max-width:640px;line-height:1.6">Un <strong>canal</strong> es por dónde tus clientes le escriben al bot: WhatsApp, Telegram, Instagram o Messenger. Elige uno de abajo, saca sus credenciales, guárdalas como secrets del Worker y apunta su webhook a la URL que ves en la tarjeta. Cuando quede listo, la tarjeta se enciende. Si no sabes por dónde empezar, deja que Claude Code te lleve de la mano:</p>
        ${renderConfigPrompt(
          "Ayúdame a conectar un canal a mi bot de Forja (WhatsApp por Cloud API oficial de Meta o por Twilio, Telegram, o Instagram/Messenger): pregúntame cuál uso, guíame paso a paso a sacar las credenciales y ponerlas como secrets del Worker, y verifica que el webhook quede recibiendo mensajes de prueba.",
          "Pega esto en Claude Code y te guía de principio a fin.",
        )}
      </div>`
      : "";
  ...
  const body = `
    <div style="display:flex;flex-direction:column;gap:20px">
      <div style="display:flex;flex-direction:column;gap:4px">
        ...
        <p class="text-muted text-[12.5px]" style="max-width:640px;line-height:1.6">Conecta los canales donde están tus clientes. Cuando un canal queda listo, su tarjeta se enciende. Los secrets se configuran con <span class="font-mono" style="color:var(--cream)">wrangler secret put NOMBRE</span> (o pídeselo a Claude Code).</p>
      </div>
      ${emptyGuide}
